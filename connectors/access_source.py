...
class AccessDataSource(DataSource):
    def __init__(self, connection_params: Dict[str, Any]):
        ...
        if not self.db_path:
            raise ValueError("Connection parameter 'db_path' is required for Access database.")
    ...
    def get_schema(self) -> Dict[str, Any]:
        ...
        for table_name in table_names:
            ...
            for col_info in columns_info:
                ...
                # col_info[12] - COLUMN_SIZE
                # col_info[13] - BUFFER_LENGTH
                # col_info[14] - DECIMAL_DIGITS
                # col_info[15] - NUM_PREC_RADIX
                # col_info[17] - IS_NULLABLE ('NO', 'YES')
                # col_info[18] - SCOPE_CATALOG
                # col_info[19] - SCOPE_SCHEMA
                # col_info[20] - SCOPE_TABLE
                # col_info[21] - SOURCE_DATA_TYPE
    ...
    def _get_primary_keys(self, cursor, table_name: str) -> List[str]:
        ...
        try:
            # Используем INFORMATION_SCHEMA, если доступно, или системные таблицы Access
            # Попробуем использовать системную таблицу MSysRelationships или pyodbc.primaryKeys
            cursor.primaryKeys(table=table_name)
            pk_info = cursor.fetchall()
            return [row[3] for row in pk_info] # row[3] - COLUMN_NAME
        except:
            # Если primaryKeys не сработает, возвращаем пустой список
            return []
